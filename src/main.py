"""
Main Orchestrator
Coordinates the entire newsletter generation process
"""
...
class NewsletterGenerator:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.scraper = ArticleScraper(self.config['sources'])
    ...
    def _load_config(self, config_path: str) -> dict:
        ...
        if not config_file.exists():
            raise FileNotFoundError(
                "No configuration file found. Please create config.yaml based on config.example.yaml"
            )
    ...
    def generate(self, hours_ago: int = 24, output_dir: str = "output") -> str:
        ...
        logger.info(f"\n[Step 1/4] Scraping articles from the last {hours_ago} hours...")
        articles = self.scraper.scrape_all_sources(hours_ago=hours_ago)

        if not articles:
            logger.error("No articles found! Check your source configurations.")
            sys.exit(1)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate an AI-powered newsletter in Morning Brew style'
    )
    parser.add_argument(
        '--config',
        default='config.yaml',
